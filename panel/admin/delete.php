						...
						<section class="category-container">
							<form action="" method="post">
								...
								<div class="part-form">
									...
									<input type="text" class="form-control" id="title" name="title" aria-describedby="title-help" value="<?php echo($item['title']) ?>" placeholder="Titre du contenu" autocomplete="off" required readonly>
								</div>
								<div class="part-form">
									<label for="url" class="form-label">Lien vers le contenu*</label>
									<input type="text" class="form-control" id="url" name="url" placeholder="Lien Direct vers le contenu" value="<?php echo($item['url']) ?>" autocomplete="off" maxlength="512" readonly>
								</div>
								<div class="part-form">
									<label for="photo" class="form-label">Lien vers l'image*</label>
									<input type="text" class="form-control" id="photo" name="photo" placeholder="Lien Direct vers l'image" autocomplete="off" value="<?php echo($item['photo']) ?>" maxlength="512" required readonly>
								</div>
                                <?php if(($_GET['category'] === 'zebra_c' || $_GET['category'] === 'decals_c' || $_GET['category'] === 'other' )) { ?>
									<div class="part-form">
										...
										<input type="text" class="form-control" id="creator_name" name="creator_name" placeholder="Nom de l'uploader" value="<?php echo($item['creator_name']) ?>" autocomplete="off" maxlength="512" required readonly>
									</div>
									<div class="part-form">
										<label for="workshop_name" class="form-label">Nom du créateur originel</label>
										<input type="text" class="form-control" id="workshop_name" name="workshop_name" placeholder="Nom du créateur originel" value="<?php echo($item['workshop_name']) ?>" autocomplete="off" required readonly>
									</div>
									<div class="part-form">
										<label for="is_enabled" class="form-label">Status de la création</label>
										<input type="text" class="form-control" id="is_enabled" name="is_enabled" placeholder="Status" value="<?php echo($item['is_enabled']) ?>" autocomplete="off" required readonly>
									</div>
                                <?php } ?>
								<label hidden>

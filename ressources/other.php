...
<main>
    <section class="header-text">
        <div>
            <h1>Autres</h1>
            <p>Une liste des autres créateurs présélectionne juste pour toi</p>
        </div>
    </section>
    ...
    <section class="download-container">
		<?php if (!empty(getItem('other', 1))) { ?>
			<?php foreach (getItem('other', 1) as $item ) { ?>
				<div class="download-card">
					...
					<div class="download-card-text">
						...
						<p>Créateur: <?php echo $item['workshop_name']; ?></p>
					</div>
					<?php if(strpos($item['photo'], "http://") === 0 || strpos($item['photo'], "https://") === 0) { ?>
        ...
        <?php } else { ?>
			<div style="grid-column-start: 1; grid-column-end: 5; display: flex; flex-direction: column; justify-content: center; align-items: center">
				<p class="text-empty">Il n'y a rien :(</p>
				<?php if(isLogged()) { ?>
					<a class="button-empty" href="../panel/index.php">Sois le premier</a>
				<?php } else { ?>
					<a class="button-empty" href="../login.php?signup">Connecte toi et sois le premier</a>
				<?php } ?>
			</div>
        <?php } ?>
